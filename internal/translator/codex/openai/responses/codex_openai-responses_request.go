...
func ConvertOpenAIResponsesRequestToCodex(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	if gjson.GetBytes(rawJSON, "background").Bool() || gjson.GetBytes(rawJSON, "store").Bool() {
		rawJSON = setCodexRequiredBool(rawJSON, "store", true)
	} else {
...
func setCodexRequiredInclude(rawJSON []byte) []byte {
	values := []string{"reasoning.encrypted_content", "web_search_call.action.sources"}
	current := gjson.GetBytes(rawJSON, "include")
	currentValues := current.Array()
	if current.IsArray() && len(currentValues) == len(values) && currentValues[0].Type == gjson.String && currentValues[0].String() == values[0] && currentValues[1].Type == gjson.String && currentValues[1].String() == values[1] {
		return rawJSON
	}
