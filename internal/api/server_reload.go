...
func (s *Server) UpdateClientsContext(ctx context.Context, cfg *config.Config) bool {
	...
	if oldCfg == nil || oldCfg.AuthDir != cfg.AuthDir {
		if errUploadedStore := s.reconfigureUploadedFileStore(cfg.AuthDir); errUploadedStore != nil {
			log.Warnf("Failed to reconfigure uploaded file store: %v", errUploadedStore)
		}
	}
...
func (s *Server) reconfigureUploadedFileStore(authDir string) error {
	if s.handlers == nil {
		return fmt.Errorf("api handlers are not configured")
	}
	if s.handlers.UploadedFileStore == nil {
		uploadStore, errUploadStore := newUploadedFileStore(authDir)
		if errUploadStore != nil {
			return errUploadStore
		}
		s.handlers.UploadedFileStore = uploadStore
		return nil
	}

	baseDir, errBaseDir := uploadedFileStoreBaseDir(authDir)
	if errBaseDir != nil {
		return errBaseDir
	}
	return s.handlers.UploadedFileStore.ReconfigureBaseDir(baseDir)
}
