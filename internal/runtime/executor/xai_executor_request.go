...
func (e *XAIExecutor) prepareResponsesRequestTo(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, stream bool, to sdktranslator.Format) (*xaiPreparedRequest, error) {
	...
	originalTranslated = preserveXAIResponsesIncludes(originalTranslated, originalPayload, from)
	...
	body = preserveXAIResponsesIncludes(body, req.Payload, from)
...
// preserveXAIResponsesIncludes prevents Codex-only response includes from
// leaking into xAI requests while retaining includes explicitly requested by
// OpenAI Responses clients.
func preserveXAIResponsesIncludes(body, source []byte, from sdktranslator.Format) []byte {
	if from == sdktranslator.FormatOpenAIResponse {
		sourceIncludes := gjson.GetBytes(source, "include")
		if !sourceIncludes.Exists() {
			updated, errDelete := sjson.DeleteBytes(body, "include")
			if errDelete == nil {
				return updated
			}
			return body
		}
		updated, errSet := sjson.SetRawBytes(body, "include", []byte(sourceIncludes.Raw))
		if errSet == nil {
			return updated
		}
		return body
	...
	filtered := []byte(`[]`)
	changed := false
	for _, include := range includes.Array() {
		if include.Type == gjson.String && include.String() == "web_search_call.action.sources" {
			changed = true
			continue
		}
		filtered, _ = sjson.SetRawBytes(filtered, "-1", []byte(include.Raw))
	}
	if !changed {
		return body
	}
	updated, errSet := sjson.SetRawBytes(body, "include", filtered)
	if errSet == nil {
		return updated
	}
	return body
