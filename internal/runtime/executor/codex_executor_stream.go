...
func newCodexStreamTrace() *codexStreamTrace {
	return &codexStreamTrace{startedAt: time.Now()}
}

func (t *codexStreamTrace) observeChunk() {
	if t == nil {
		return
	}
	t.lastUpstreamChunkAt = time.Now()
	t.upstreamChunkCount++
}

func (t *codexStreamTrace) logAbnormalEnd(ctx context.Context, baseModel string, authID string, upstreamURL string, headers http.Header) {
	if t == nil {
		return
	}
	var ctxErr error
	...
	if (t.seenCompleted || t.seenTerminal) && t.streamErr == nil && ctxErr == nil {
		return
	}
	fields := log.Fields{
		"model":                   baseModel,
		...
		"seen_response_completed": t.seenCompleted,
		"seen_terminal_error":     t.seenTerminal,
		"upstream_chunk_count":    t.upstreamChunkCount,
		"duration_ms":             time.Since(t.startedAt).Milliseconds(),
	}
	for key, value := range codexStreamTraceHeaderFields(headers) {
		fields[key] = value
	}
	if !t.lastUpstreamChunkAt.IsZero() {
		fields["last_upstream_chunk_at"] = t.lastUpstreamChunkAt.Format(time.RFC3339Nano)
		fields["idle_since_last_chunk_ms"] = time.Since(t.lastUpstreamChunkAt).Milliseconds()
	}
	if t.streamErr != nil {
		fields["stream_error"] = t.streamErr.Error()
	}
	if ctxErr != nil {
...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	body = normalizeCodexResponsesCompatibility(body)
	...
	trace := newCodexStreamTrace()
	traceOwnedByCaller := true
	defer func() {
		if traceOwnedByCaller {
			trace.logAbnormalEnd(ctx, baseModel, authID, url, httpResp.Header)
		}
	}()
	...
	if buffering {
		for scanner.Scan() {
			...
			if transformed, ok := grokbuild.TransformKeepaliveSSELine(translatedLine, isGrokClient); ok {
				...
				if streamErr, terminalBody, ok := codexTerminalFailureErr(data); ok {
					trace.seenTerminal = true
					trace.streamErr = streamErr
					closeBootstrapBody()
					if errClearReplay := clearCodexReasoningReplayOnInvalidSignature(ctx, replayScope, streamErr.StatusCode(), terminalBody); errClearReplay != nil {
						trace.streamErr = errClearReplay
		...
		if !streamStarted && bootstrapTerminalErr == nil {
			...
			if errScan := scanner.Err(); errScan != nil {
				...
				if ctx.Err() != nil {
					trace.streamErr = ctx.Err()
					return nil, ctx.Err()
				}
				trace.streamErr = errScan
	...
	go func() {
		defer close(out)
		defer trace.logAbnormalEnd(ctx, baseModel, authID, url, httpResp.Header)
		defer func() {
			if errClose := httpResp.Body.Close(); errClose != nil {
				log.Errorf("codex executor: close response body error: %v", errClose)
			}
