...
// StreamBootstrapBufferingEnabled reports whether initial Codex stream events should be buffered.
// The zero value preserves the historical enabled behavior; an explicitly configured false value
// disables buffering.
func (c CodexConfig) StreamBootstrapBufferingEnabled() bool {
	return !c.streamBootstrapBufferingSet || c.StreamBootstrapBuffering
}

// SetStreamBootstrapBuffering sets the buffering policy as an explicit configuration value.
func (c *CodexConfig) SetStreamBootstrapBuffering(enabled bool) {
	if c == nil {
		return
	}
	c.StreamBootstrapBuffering = enabled
	c.streamBootstrapBufferingSet = true
}

// UnmarshalYAML preserves whether stream-bootstrap-buffering was explicitly configured so the
// zero-value CodexConfig can default to buffering without losing an explicit false setting.
func (c *CodexConfig) UnmarshalYAML(value *yaml.Node) error {
	type plainCodexConfig CodexConfig
	var decoded plainCodexConfig
	...
	*c = CodexConfig(decoded)
	...
	for i := 0; i+1 < len(value.Content); i += 2 {
		key := value.Content[i]
		if key != nil && key.Value == "stream-bootstrap-buffering" {
			c.streamBootstrapBufferingSet = true
			break
		}
	}
...
// UnmarshalJSON preserves explicit false values for JSON-based configuration updates.
func (c *CodexConfig) UnmarshalJSON(data []byte) error {
	type plainCodexConfig CodexConfig
	var decoded plainCodexConfig
	...
	*c = CodexConfig(decoded)
	...
	if errFields := json.Unmarshal(data, &fields); errFields != nil {
		return errFields
	}
	_, c.streamBootstrapBufferingSet = fields["stream-bootstrap-buffering"]
	return nil
}
...
type OpenAICompatibility struct {
	...
	// ResponsesPassthrough allows selected OpenAI-compatible providers to receive
	// OpenAI Responses API requests directly at /responses instead of translating
	// them to /chat/completions. Only enable this for providers known to support
	// Responses background retrieve/resume.
