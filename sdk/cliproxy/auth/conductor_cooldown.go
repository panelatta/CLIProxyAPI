...
const credentialExhaustionFallbackCooldown = 30 * time.Minute
...
func resultErrorForProvider(provider string, err error) (*Error, *time.Duration) {
	...
	if resultErr == nil || !strings.EqualFold(strings.TrimSpace(provider), "codex") || !isCredentialExhaustionErrorMessage(resultErr.Message) {
		return resultErr, retryAfter
	}
	resultErr.HTTPStatus = http.StatusTooManyRequests
	resultErr.Retryable = true
	if retryAfter == nil {
		value := credentialExhaustionFallbackCooldown
		retryAfter = &value
	}
	return resultErr, retryAfter
}
