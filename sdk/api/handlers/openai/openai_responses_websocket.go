...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		if pinnedAuthID != "" {
			if pinnedAuth, ok := sessionAuthByID(pinnedAuthID); ok && coreauth.WebsocketsEnabled(pinnedAuth) {
		...
		pinnedWebsocketContinuation := nativeWebsocketPassthrough && requestRequiresCurrentUpstreamWebsocket &&
		...
		replayPinnedAuthFailure := func(errMsg *interfaces.ErrorMessage) bool {
			return pinnedWebsocketContinuation && shouldReplayResponsesWebsocketPinnedAuthFailure(errMsg)
		}
