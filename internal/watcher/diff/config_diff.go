...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	oldStreamBootstrapBuffering := oldCfg.Codex.StreamBootstrapBufferingEnabled()
	newStreamBootstrapBuffering := newCfg.Codex.StreamBootstrapBufferingEnabled()
	if oldStreamBootstrapBuffering != newStreamBootstrapBuffering {
		changes = append(changes, fmt.Sprintf("codex.stream-bootstrap-buffering: %t -> %t", oldStreamBootstrapBuffering, newStreamBootstrapBuffering))
	}
	...
	if !equalAccessAPIKeyEntryForceModels(oldCfg.APIKeyEntries, newCfg.APIKeyEntries) {
		changes = append(changes, "api-keys: force-model policies updated")
	}
...
func equalAccessAPIKeyEntryForceModels(a, b []config.AccessAPIKeyEntry) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
		if strings.TrimSpace(a[i].ForceModel) != strings.TrimSpace(b[i].ForceModel) {
			return false
		}
	}
	return true
