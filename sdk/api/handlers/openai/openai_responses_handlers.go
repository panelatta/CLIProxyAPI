...
func (h *OpenAIResponsesAPIHandler) rememberResponseRoute(c *gin.Context, responseID string, tracker *selectedAuthTracker) {
	...
	if responseID == "" || h == nil || h.responseRoutes == nil || tracker == nil {
		return
	}
	authID := tracker.Get()
...
func (h *OpenAIResponsesAPIHandler) GetResponse(c *gin.Context) {
	...
	upstreamResp, errRequest := h.AuthManager.ResponsesHTTPRequest(cliCtx, auth, responseID, c.Request.URL.Query())
	if errRequest != nil {
		...
		cliCancel(errRequest)
		return
	}
	defer func() {
		if upstreamResp != nil && upstreamResp.Body != nil {
			if errClose := upstreamResp.Body.Close(); errClose != nil {
				log.Debugf("responses resume: close upstream response body: %v", errClose)
			}
		}
	}()
...
func (h *OpenAIResponsesAPIHandler) writeUpstreamResponsesHTTPResponse(c *gin.Context, upstreamResp *http.Response) {
	...
	if isStream {
		...
		for {
			n, errRead := upstreamResp.Body.Read(buf)
			if n > 0 {
			...
			if errRead != nil {
				if errRead != io.EOF {
					_, _ = fmt.Fprintf(c.Writer, "\nevent: error\ndata: %s\n\n", handlers.BuildOpenAIResponsesStreamErrorChunk(http.StatusBadGateway, errRead.Error(), 0))
					flusher.Flush()
				}
				return
			}
...
func (h *OpenAIResponsesAPIHandler) handleStreamingResponse(c *gin.Context, rawJSON []byte, usedFileIDs []string) {
	...
	framer := &responsesSSEFramer{failureEvent: failureEvent, isCodexClient: isCodexClient, observePayload: h.observeResponseRoutePayload(c, selectedAuth, responsesShouldRememberRoute(rawJSON))}
	...
	for {
		...
		case errMsg, ok := <-errChan:
			...
			if safeErrMsg != nil && framer.dataFrames > 0 {
				...
				close(pendingErrors)
				h.forwardResponsesStream(c, flusher, finishStream, make(chan []byte), pendingErrors, framer)
				return
			}
		...
		case chunk, ok := <-dataChan:
			if !ok {
				...
				if framer.dataFrames > 0 {
					...
					if framer.terminalError != nil {
						...
						finishStream(framer.terminalError.Error)
						return
					}
					if errMsg == nil {
						finishStream(nil)
			...
			h.forwardResponsesStream(c, flusher, finishStream, dataChan, errChan, framer)
			return
		}
	}
}
