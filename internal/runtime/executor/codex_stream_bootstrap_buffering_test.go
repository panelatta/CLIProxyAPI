...
func codexBufferingConfig(enabled bool) *config.Config {
	cfg := &config.Config{}
	cfg.Codex.SetStreamBootstrapBuffering(enabled)
	return cfg
}

func TestCodexStreamBootstrapBufferingDefaultsEnabled(t *testing.T) {
	if !(&config.Config{}).Codex.StreamBootstrapBufferingEnabled() {
		t.Fatal("zero-value Codex config must preserve default bootstrap buffering")
	}
	if codexBufferingConfig(false).Codex.StreamBootstrapBufferingEnabled() {
		t.Fatal("explicit stream-bootstrap-buffering=false must disable buffering")
	}
}
...
func TestCodexExecutor_BootstrapBuffering_ExplicitlyDisabledPassthrough(t *testing.T) {
...
func TestCodexWebsocketsExecutor_BootstrapBuffering_ExplicitlyDisabledPassthrough(t *testing.T) {
