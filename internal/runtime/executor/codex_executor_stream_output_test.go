...
func TestCodexExecutorExecute_ResponseDoneCompletesAndPatchesOutput(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"ok\"}]},\"output_index\":0}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.done\",\"response\":{\"id\":\"resp_1\",\"object\":\"response\",\"created_at\":1775555723,\"status\":\"completed\",\"model\":\"gpt-5.5\",\"output\":[],\"usage\":{\"input_tokens\":8,\"output_tokens\":28,\"total_tokens\":36}}}\n\n"))
	}))
	defer server.Close()
...
func TestCodexExecutorExecuteStreamMissingCompletionIsRequestScoped(t *testing.T) {
	...
	if err != nil {
		if result != nil {
			t.Fatal("expected nil stream result for a buffered pre-commit failure")
		}
		if se, ok := err.(interface{ StatusCode() int }); !ok || se.StatusCode() != http.StatusRequestTimeout {
			t.Fatalf("ExecuteStream error = %T %v, want status %d", err, err, http.StatusRequestTimeout)
		}
		if !strings.Contains(err.Error(), "before response.completed") {
			t.Fatalf("ExecuteStream error = %q, want incomplete-stream diagnostic", err.Error())
		}
		return
	}
...
func TestCodexExecutorTransportFailureBeforeTerminalIsRequestScoped(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg := &config.Config{}
			cfg.Codex.SetStreamBootstrapBuffering(false)
...
func TestCodexExecutorExecuteStreamDoesNotAppendTimeoutAfterTerminalCapacityError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("event: response.created\n"))
		_, _ = w.Write([]byte(`data: {"type":"response.created","response":{"id":"resp_1","model":"gpt-5.5"}}` + "\n\n"))
		_, _ = w.Write([]byte("event: error\n"))
		_, _ = w.Write([]byte(`data: {"type":"error","error":{"type":"service_unavailable_error","code":"server_is_overloaded","message":"Our servers are currently overloaded. Please try again later.","param":null},"sequence_number":2}` + "\n\n"))
	}))
	defer server.Close()
	...
	if streamErr == nil {
		t.Fatal("expected upstream terminal capacity error in stream chunks")
	}
	if !strings.Contains(streamErr.Error(), "server_is_overloaded") {
		t.Fatalf("stream error missing upstream capacity error: %v", streamErr)
	}
	if strings.Contains(streamErr.Error(), "request_timeout") || strings.Contains(streamErr.Error(), "stream disconnected before completion") {
		t.Fatalf("stream contains synthetic timeout after terminal error: %v", streamErr)
	}
}
...
func TestCodexExecutorExecuteStream_ResponseDoneCompletesAndPatchesOutput(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"ok\"}]},\"output_index\":0}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.done\",\"response\":{\"id\":\"resp_1\",\"object\":\"response\",\"created_at\":1775555723,\"status\":\"completed\",\"model\":\"gpt-5.5\",\"output\":[],\"usage\":{\"input_tokens\":8,\"output_tokens\":28,\"total_tokens\":36}}}\n\n"))
...
func TestCodexExecutorExecuteStream_ClosedBeforeCompletedReturnsError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
	...
	if got := streamErr.Error(); !strings.Contains(got, "before response.completed") {
		t.Fatalf("stream error = %q, want incomplete-stream diagnostic", got)
	}
	if se, ok := streamErr.(interface{ StatusCode() int }); !ok || se.StatusCode() != http.StatusRequestTimeout {
		t.Fatalf("stream error status = %#v, want 408", streamErr)
	}
}
