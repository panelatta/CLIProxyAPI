...
func newUploadedFileStore(authDir string) (*internalfiles.Store, error) {
	baseDir, err := uploadedFileStoreBaseDir(authDir)
	if err != nil {
		return nil, err
	}

	store := internalfiles.NewStore(baseDir)
...
func uploadedFileStoreBaseDir(authDir string) (string, error) {
	...
	if strings.TrimSpace(resolvedAuthDir) == "" {
		...
		if errHome != nil {
			return "", fmt.Errorf("resolve uploaded file store directory: %w", errHome)
		}
