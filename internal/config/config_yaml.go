...
func SaveConfigPreserveComments(configFile string, cfg *Config) error {
	...
	// APIKeyEntries is runtime-only in Config so regular yaml.Marshal emits only
	// []string APIKeys. Replace the generated node with the metadata-aware shape
	// before merging, while keeping legacy string YAML when no metadata is set.
	replaceAPIKeysNode(generated.Content[0], persistCfg.APIKeys, persistCfg.APIKeyEntries)
